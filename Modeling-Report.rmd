...
# linearHypothesis(bic_step_both.model, L_temp_location)
# X_bic = model.matrix(bic_step_both.model)
# f_score_to_test_temp_location = getFStat(bic_step_both.model, X_bic, L_temp_location, matrix(c(0, 0), ncol=1))
...
In regards to our research questions, we were able to conclude that our hypothesis that $\beta_{temp}$ is different in different locations, at least for the fall season by using a nonparametric bootstrap f-test to determine that the interaction terms $\beta_{temp:locationseoul}$ and $\beta_{temp:locationwashington_dc}$ are not 0. We also were able to conclude with a nonparametric bootstrap f-test that the covariates found to be insignificant in the model selected by BIC bidirectional selection are actually in fact significant as a group.
