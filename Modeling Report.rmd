...
```{r}
kable(data.frame(mean=mean(df$cnt), var=var(df$cnt)), caption="Mean and Variance of cnt")
```
...
Again, we see that when training on full data, the RMSE (and log-likelihood) of the complex models are better than that of the simpler models. An interesting observation is that even though the RMSE of the poisson models are better than the equivalent negative binomial models, they have much worse log-likelihoods, approx 10x worse. The log-ols models gives us the best log-likelihood and MAE providing us justification for choosing this over the other models in our further analysis. The following plots show us the relationship between predicted values and residuals. We observe that residuals are not evenly scattered about 0.
...
Here's a closer look at the log-ols model, with the fit against $cnt$ and $log(1+cnt)$ respectively. We observe that besides a mode of errors when $cnt$ is extremely low, the log-ols model has a good fit against $log(cnt+1)$. We see a mode of errors, shown by the straight lines with negative coefficients, that occurs when $cnt$ is extremely low. We observe that our residuals are evenly scattered around the x-axis for our log-normal model, with slight heteroskedasticity.
...
num_coeffs_df = data.frame(
  model = c("full", "bic_bidirectional", "aic_bidirectional"),
  num_coeff = c(
    size(full.model$coefficients)[2],
    size(bic_step.model$coefficients)[2],
    size(aic_step.model$coefficients)[2]
  )
)
kable(num_coeffs_df)
```

The AIC and BIC model both shrink the model by the same 23 parameters. The parameters removed are:

```{r}
all_coeff_names = names(full.model$coefficients)
bic_coeff_names = names(bic_step.model$coefficients)
aic_coeff_names = names(aic_step.model$coefficients)
setdiff(all_coeff_names, bic_coeff_names)
```
The selection criteria means that the triple interaction term of $temp*hr*isWorkday$ is removed. The resulting model is:

```{r}
summary(bic_step.model)
```
...
```{r}
pvals = summary(bic_step.model)$coefficients[, 4]
...
```{r}
get_p_value_from_bootstrap = function(model, X, L, c) {
  B = 2
  ...
  get_p_value_from_bootstrap = sum(vect_of_bootstrapped_f_scores > f_score_to_test[1])
}
```
...
```{r}
B = 5
n = dim(X_bic)[1] # number of rows
vect_of_bootstrapped_f_scores = rep(0, B)

log_cnt = log(df$cnt + 1)
X_bic_plus_log_cnt = cbind(X_bic, log_cnt = log_cnt)
estimated_c = L_temp_location %*% (summary(bic_step.model)$coefficients[,1] %>% as.matrix(ncol=1))

for (i in 1:B) {
  set.seed(i)
  random_rows = sample(1:n, n, replace=TRUE)
  X_bic_plus_log_cnt_random_sample = X_bic_plus_log_cnt[random_rows, ]
  lm_bootstrap = lm(log_cnt ~ ., data=as.data.frame(X_bic_plus_log_cnt_random_sample))
  X_bic_random_sample = subset(X_bic_plus_log_cnt_random_sample, select=-(log_cnt)) # Remove log_cnt
  bootstrapped_f = getFStat(lm_bootstrap, X_bic_random_sample, L_temp_location, estimated_c)
  vect_of_bootstrapped_f_scores[i] = bootstrapped_f
}
vect_of_bootstrapped_f_scores
```
...
#   set.seed(i)
#   random_rows = sample(1:n, n, replace=TRUE)
#   random_sample_of_X_plus_log_charges_minus_intercept = X_plus_log_charges_minus_intercept[random_rows, ]
#   lm_bootstrap = lm(log_charges ~ ., data=random_sample_of_X_plus_log_charges_minus_intercept)
#   random_sample_of_X_minus_intercept = subset(random_sample_of_X_plus_log_charges_minus_intercept, select=-(log_charges))
#   random_sample_of_X = cbind(intercept=1, random_sample_of_X_minus_intercept)
#   bootstrapped_f = getFStat(lm_bootstrap, random_sample_of_X, L, beta_naut)
#   vect_of_bootstrapped_f_scores[i] = bootstrapped_f
# }
```
