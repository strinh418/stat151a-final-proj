...
Here's a closer look at the log-ols model, with the fit against $cnt$ and $log(1+cnt)$ respectively. We observe that besides a mode of errors when $cnt$ is extremely low, the log-ols model has a good fit against $log(cnt+1)$.
...
The AIC and BIC model both shrink the model by 23 parameters. The parameters removed are:
...
setdiff(aic_coeff_names, bic_coeff_names)
setdiff(all_coeff_names, bic_coeff_names)
```
The AIC and BIC models both coincide. The selection criteria means that the triple interaction term of $temp*hr*isWorkday$ is removed.
